...
def read_deseq2_file(file_path):
    ...
    if not Path(file_path).exists():
        raise WormcatError(
            f"Attempting to read a non-existent file: {file_path}",
    ...
    if missing_columns:
        raise WormcatError(
            f"{get_name_from_file_path(file_path)} file is missing required columns: {missing_columns}",
...
def extract_run_number(data_file_nm):
    ...
    if match:
        ...
        raise WormcatError(
            f"Invalid file name: {data_file_nm}. It must end with 'run_?????.csv' where '?????' are any 5 digits.",
