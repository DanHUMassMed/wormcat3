...
class EnrichmentAnalyzer:
    ...
    def calculate_category_enrichment_scores(
        self,
        ...
        if not isinstance(p_adjust_method, PAdjustMethod):
            raise WormcatError(
                f"Invalid p_adjust_method: {p_adjust_method}. Must be a valid PAdjustMethod.",
        ...
        for category in self.categories:
            ...
            fisher_cat_adjusted_df = self._adjust_pvalues(
                fisher_cat_df, category, method=p_adjust_method.value, threshold=p_adjust_threshold
            )
    ...
    def _run_fisher_test(self, gene_set_and_categories_df, category):
        ...
        for _, row in merged_categories_df.iterrows():
            ...
            if pd.isna(rgs_value) or pd.isna(ac_value):
                ...
                contingency_table = self.contingency_func(
                    rgs_value, total_gene_set_count, ac_value, total_annotations_count
                )
        ...
        # Sort and save
        fisher_cat_df = fisher_cat_df.sort_values(
            by=["PValue", "Category"],
    ...
    def _adjust_pvalues(self, fisher_cat_df, category, *, method="bonferroni", threshold=0.01):
        ...
        if method not in {"bonferroni", "fdr_bh"}:
            raise WormcatError(
                "Invalid method. Choose either 'bonferroni' or 'fdr_bh'.",
                ErrorCode.INVALID_VALUE.to_dict(),
            )
        ...
        # Sort by p-value
        fisher_cat_adjusted_df.sort_values(
            by=["PValue", "Category"],
            key=lambda col: col.str.lower() if col.name == "Category" else col,
            inplace=True,
        )
