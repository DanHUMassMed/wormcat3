...
class GSEAAnalyzer:
        ...
        *,
        ...
        if isinstance(ranked_genes, pd.DataFrame):
            ...
            if not required_columns.issubset(ranked_genes.columns):
                raise WormcatError(
                    f"ranked_genes DataFrame must contain columns: {required_columns}",
        ...
        try:
            ...
            results_df = (
                pd.DataFrame(results_list, columns=["Term", "FDR", "ES", "NES", "P-value", "Tag %"])
                .sort_values("FDR")
            ...
            raise WormcatError(
                f"GSEA analysis failed: {str(e)}",
                ErrorCode.INTERNAL_ERROR.to_dict(),
                origin="GSEAAnalyzer.run_preranked_gsea",
            )
    ...
    def create_ranked_list(self, deseq2_output_df):
        ...
        assert not deseq2_copy["ID"].isna().any(), (
            "Column 'ID' contains NaN values, which are not allowed for identifiers"
        )
