...
class Wormcat:
    ...
    def __init__(
        ...
        while not is_validate_directory:
            ...
            is_validate_directory, self.working_dir_path = file_util.validate_directory_path(
                working_dir_path, validation_indicator=True
            )
    ...
    def perform_enrichment_analysis(
        ...
        *,
        ...
        # Add annotations
        gene_set_and_categories_df, genes_not_matched_df = self.annotation_manager.segment_genes_by_annotation_match(
            gene_set_list, gene_type
        )
        ...
        if background_list is not None:
            ...
            if background_type != gene_type:
                raise WormcatError(
                    "Gene Set Type and Background Type MUST be the same. {gene_type}!={background_type}",
                    ErrorCode.CONSTRAINT_VIOLATION.to_dict(),
                )

            background_df, background_not_annotated_df = self.annotation_manager.segment_genes_by_annotation_match(
                background_list, background_type
            )
            ...
            if not background_not_annotated_df.empty:
                background_not_annotated_path = (
                    Path(self.working_dir_path) / f"background_not_annotated_{self.run_number}.csv"
                )
        ...
        self.analyzer = EnrichmentAnalyzer(background_df, self.working_dir_path, self.run_number)
    ...
    def analyze_and_visualize_enrichment(
        self,
        gene_set_input: Union[str, list],
        background_input: Optional[Union[str, list]] = None,
        *,
        ...
        test_results = self.perform_enrichment_analysis(
            gene_set_input,
            background_input,
            p_adjust_method=p_adjust_method,
            p_adjust_threshold=p_adjust_threshold,
