...
class AnnotationsManager:
    ...
    def _load_annotations(self):
        ...
        try:
            ...
            if df.empty:
                raise WormcatError(
                    f"Annotation file '{self.annotation_file_path}' is empty.",
                    ErrorCode.FILE_LOAD_FAILED.to_dict(),
                )
            return df
        except Exception as e:
            raise WormcatError(
                f"Failed to load annotation file: {e}",
                ErrorCode.FILE_LOAD_FAILED.to_dict(),
    ...
    def get_gene_id_type(self, gene_set):
        ...
        valid_genes = [
            g
            for g in gene_set[1:]  # Skip the first row
    ...
    def add_annotations(self, gene_set_list, gene_type):
        ...
        if gene_type not in self.annotations_df.columns:
            raise WormcatError(
                f"Column '{gene_type}' not found in the DataFrame.",
                ErrorCode.INVALID_VALUE.to_dict(),
            )
    ...
    def segment_genes_by_annotation_match(self, gene_set_list, gene_type):
        ...
        if gene_type not in gene_set_df.columns:
            raise WormcatError(
                f"'{gene_type}' MUST be a column in the Gene Set DataFrame.",
                ErrorCode.MISSING_FIELD.to_dict(),
            )
        if gene_type not in self.annotations_df.columns:
            raise WormcatError(
                f"Incorrect '{gene_type}' name. Column not found in {self.annotation_name()} file.",
                ErrorCode.MISSING_FIELD.to_dict(),
            )
    ...
    def category_to_gmt_format(self, category, id_col_nm="Wormbase.ID"):
        ...
        if missing_cols:
            raise WormcatError(
                f"Incorrect column names: {', '.join(missing_cols)} {self.annotation_name()} file does not have these columns.",
                ErrorCode.MISSING_FIELD.to_dict(),
            )

        if category_df[id_col].isna().any():
            raise WormcatError(
                f"{self.annotation_name()} dataframe Column '{id_col}' contains NaN values",
                ErrorCode.INVALID_VALUE.to_dict(),
            )
