...
class WormcatExcel:
    ...
    def __init__(self, significance_threshold: float = 0.05, p_value_thresholds: Optional[List[float]] = None):
        """
        ...
        self.excel_formats: Optional[List[Any]] = None
    ...
    def extract_csv_files(excel_path: str, csv_file_path: str) -> Dict[str, str]:
        ...
        try:
            ...
            raise WormcatError(
                f"File [{excel_path}] is not a valid Excel file.",
    ...
    def create_summary_spreadsheet(self, wormcat_out_path: str, annotation_file: str, out_xsl_file_nm: str) -> None:
        """
        ...
        if not wormcat_path.exists():
            raise WormcatError(
                f"Wormcat output path not found: {wormcat_out_path}",
    ...
    def _create_category_summary(self, data: pd.DataFrame, category_name: str) -> pd.DataFrame:
        ...
        if category_name not in data.columns:
            raise WormcatError(
                f"Category '{category_name}' not found in data columns",
    ...
    def _process_category_file_row(self, row: pd.Series, sheet: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            cat_results.rename(
                columns={"Category": label_category, "RGS": label_rgs, "PValue": label_pvalue},
                inplace=True,
            )
    ...
    def _create_legend(self, writer: pd.ExcelWriter) -> None:
        ...
        if self.excel_formats is None:
            return

        # Apply conditional formatting
        worksheet.conditional_format(
            sheet_range,
            {"type": "cell", "criteria": "=", "value": 0, "format": self.excel_formats[0]},
        )
    ...
    def _process_category_files(self, files_to_process: pd.DataFrame, annotation_file: str, out_data_xlsx: str) -> None:
        """
