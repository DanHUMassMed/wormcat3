...
def preprocess_bubble_data(data_file_path, add_calibration=False):
    ...
    try:
        ...
        bubbles_data["bubbles_z"] = round(
            scale_for_visualization
            * (bubbles_data["PValue"] - bubbles_data["PValue"].mean())
            / bubbles_data["PValue"].std(),
            2,
        )
        ...
        raise WormcatError(
            f"Error/Warning in create Bubble Chart: CSV file not found: {data_file_path}",
            ErrorCode.FILE_NOT_FOUND.to_dict(),
        )
    except pd.errors.EmptyDataError:
        raise WormcatError(
            f"Error/Warning in create Bubble Chart: The CSV file is empty: {data_file_path}",
            ErrorCode.FILE_LOAD_FAILED.to_dict(),
        )
    except pd.errors.ParserError:
        raise WormcatError(
            f"Error/Warning in create Bubble Chart: Error parsing CSV file: {data_file_path}",
            ErrorCode.FILE_LOAD_FAILED.to_dict(),
        )
    except KeyError as e:
        raise WormcatError(
            f"Error/Warning in create Bubble Chart: Required column missing in CSV file: {e}",
            ErrorCode.FILE_LOAD_FAILED.to_dict(),
        )
...
def order_categories_by_column(df, category_column="Category", order_by_column="PValue", ascending=True):
    ...
    if df is None or df.empty:
        raise WormcatError(
            "While creating bubble plot found DataFrame empty or None",
            ErrorCode.INVALID_STATE.to_dict(),
        )

    if category_column not in df.columns:
        raise WormcatError(
            f"While creating bubble plot required column '{category_column}' not found in DataFrame",
            ErrorCode.INVALID_STATE.to_dict(),
        )

    if order_by_column not in df.columns:
        raise WormcatError(
            f"While creating bubble plot required column '{order_by_column}' not found in DataFrame",
            ErrorCode.INVALID_STATE.to_dict(),
        )
...
def generate_bubble_plot(
    df, svg_file_path, plot_title=cs.DEFAULT_TITLE, width=cs.DEFAULT_WIDTH, height=cs.DEFAULT_HEIGHT
):
    """
    ...
    if df is None or df.empty:
        raise WormcatError(
            "While Generating Bubble Plot found DataFrame empty or None",
            ErrorCode.INVALID_STATE.to_dict(),
        )
    ...
    if missing_columns:
        raise WormcatError(
            f"While Generating Bubble Plot found required columns missing in DataFrame: {missing_columns}",
...
def create_bubble_chart(dir_path: str, data_file_nm: str, plot_title="RGS", add_calibration=False) -> None:
    try:
        ...
        def height_fun(x: float) -> float:
            return min(0.1 * x + 6.3, 25.0)

        height = height_fun(float(bubbles_data_len))
